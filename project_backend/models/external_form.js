...
// Define the external form schema
const externalSchema = new mongoose.Schema({
    ...
    domain: {
        ...
        maxlength: [100, 'Domain name should not exceed 100 characters']
    },
    projectTitle: {
        type: String,
    ...
    member2Roll: {
        type: String,
        match: [/^\d{7}[A-Za-z]{2}\d{3}$/, 'Member 2 roll number must follow the format 7376221CS159']
    },
    email2: {
        type: String,
    ...
    member3Roll: {
        type: String,
        match: [/^\d{7}[A-Za-z]{2}\d{3}$/, 'Member 3 roll number must follow the format 7376221CS159']
    },
    email3: {
        type: String,
...
module.exports = mongoose.model('External_form', externalSchema);
